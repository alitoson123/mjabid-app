...
const LG={open:false,screen:'list',kind:'official',tab:'overview',data:null,league:null,busy:false,timer:null,requestId:null,clockOffset:0,drafts:{},chatId:null,chatLast:null,viewportCleanup:null,routeSeq:0,readSeq:0,readController:null,animation:null,actionError:'',registration:null,seatIntent:null,seatChoice:null,bracketScope:'current'};
...
function lgPerson(p){return `<div class="lg-person"><span class="lg-avatar">${p.bot?lgSvg('people'):avatarHTML(String(p.avatar||'').replace(/[<>"']/g,''),36)}</span><div><strong>${p.bot?'كمبيوتر':lgEsc(p.name)}</strong><small>${p.bot?(Number.isInteger(p.index)?'مقعد الكمبيوتر '+lgNum(p.index+1):'مقعد الكمبيوتر'):lgEsc(typeof rankFor==='function'?rankFor(p.rank||0).name:'')}</small></div></div>`;}
...
async function lgOpen(id,tab='overview'){
 ...
 lgViewport();document.getElementById('leagueHub').querySelector('#lgBack')?.focus();lgNavigate(id?'detail':'list',{tab,...(id?{league:{id}}:{})},true);
...
function lgDrawInfo(l){
 const d=l.draw;if(!d)return '';
 if(!d.seats)return '<p class="lg-note">السعة حد أقصى للتسجيل؛ تتحدد الأدوار عند البداية بحسب المسجلين. يلزم مسجّل بشري واحد على الأقل.</p>';
 return `<section class="lg-alert"><strong>${d.confirmed?'حجم البطولة المعتمد':'الحجم المتوقع حسب المسجلين الآن'}</strong><p>${lgNum(d.teams)} فرق · ${lgNum(d.rounds)} أدوار · ${lgNum(d.seats)} مقاعد${d.bots?' · '+lgNum(d.bots)+' كمبيوتر':''}</p><small>${d.byes?'تأهل مباشر بالقرعة لـ '+lgNum(d.byes)+' من الفرق في الدور الأول. ':'لا تأهل مباشر في الدور الأول. '}${d.confirmed?'القرعة ثابتة لهذه البطولة.':'السعة '+lgNum(l.capacity)+' حد أقصى؛ يُحسم العدد والقرعة عند موعد البداية بعد إغلاق التسجيل.'}</small></section>`;
}
function lgCard(l){return `<article class="lg-card"><div class="lg-kicker"><span>${lgEsc(l.kind==='official'?'دوريات مجابيد':l.kind==='private'?'دوري خاص':'دوري عام')}</span><span class="lg-badge">${lgStatus[l.status]||''}</span></div><h3>${lgEsc(l.name)}</h3>${lgRankRule(l)}<div class="lg-prizes"><div>🥇 لكل لاعب<b>${lgNum(l.prizes.firstEach)}</b><small>ذهب · البطل</small></div><div>🥈 لكل لاعب<b>${lgNum(l.prizes.secondEach)}</b><small>ذهب · الوصيف</small></div></div><div class="lg-data"><div><b>${lgNum(l.count)}/${lgNum(l.capacity)}</b><small>لاعبون مسجّلون</small></div><div><b>${lgNum(l.fee)}</b><small>رسم الدخول</small></div><div><b>${l.goal?lgNum(l.goal):'رزمة'}</b><small>نهاية المباراة</small></div></div>${lgDrawInfo(l)}<p class="lg-note">${lgEsc(lgDate(l.start))} · توقيت السعودية<br>الجوائز الحالية من الرسوم المحصّلة؛ تتحدث مع التسجيل.</p><footer><button class="lg-strong" data-lg="detail" data-id="${l.id}">دخول الدوري</button></footer></article>`;}
function lgRender(){
...
function lgMatchMembers(l,m,teamIndex){
 const team=l.teams.find(t=>t.id===m.teams[teamIndex]);if(!team)return [];
 return team.members.map((p,n)=>{
  const index=teamIndex+n*2,vacant=m.snapshot?.vacancies?.some(v=>v.index===index&&v.bot);
  return {...p,index,bot:!!(p.bot||vacant)};
 });
}
function lgTeam(l,id,m=null){
 const t=l.teams.find(t=>t.id===id);if(!t)return '<p class="lg-note">بانتظار المتأهل</p>';
 const people=m?lgMatchMembers(l,m,m.teams.indexOf(id)):t.members;
 return people.map(p=>`<div class="lg-bracket-seat">${lgPerson(p)}${m&&!m.done&&!m.bye&&p.bot?`<button data-lg="participate" data-match="${lgEsc(m.id)}" data-index="${p.index}" aria-label="شارك مكان الكمبيوتر في المقعد ${p.index+1}">شارك</button>`:''}</div>`).join('');
}
function lgBracketSummary(l){
 const humans=Number(l.count)||0,bots=Number(l.botCount)||0;
 return `<div class="lg-alert"><strong>${lgNum(humans)} مسجّل بشري · ${lgNum(bots)} مقعد كمبيوتر في القرعة</strong>${humans===1?'<p>بدأ الدوري بمسجّل بشري واحد، وعدد مقاعد الكمبيوتر موضح أعلاه. يمكنك مشاهدة المباريات أو طلب المشاركة مكان الكمبيوتر.</p>':''}<small>«شارك» يفتح المشاهدة أولًا؛ تختار مقعدًا وتؤكد رسوم الدخول بعد تحقق السيرفر. المقاعد المعروضة تتحدث مع حالة المباراة.</small></div>`;
}
...
function lgSeatOptions(l,watching){
 ...
 const v=ONL.view,choice=LG.seatChoice,selected=choice&&choice.league===l.id&&choice.match===watching.id&&choice.uid===FB.user?.uid&&Date.now()<choice.expires?choice.index:-1;
 return `<section class="lg-card lg-seat-options"><h3>انضم مكان الكمبيوتر</h3><p class="lg-note">اختر المقعد؛ تكمل بنفس الفريق واليد والكومة. رسوم الدخول ${lgNum(l.fee)} ذهب. يُراجع السيرفر رتبتك وتسجيلك وتوفر المقعد قبل الخصم.</p>${selected>=0?'<p class="lg-note">اخترت المقعد '+lgNum(selected+1)+(v.humans.includes(v.seats[selected])?'، وقد شُغل بالفعل. اختر مقعدًا آخر إن توفر.':'؛ أكّد المشاركة بزر المقعد أدناه.')+'</p>':''}${lgRankRule(l,true)}${v.seats.map((pid,index)=>v.humans.includes(pid)?'':`<button data-lg="seat" data-match="${lgEsc(watching.id)}" data-index="${index}" class="${index===selected?'lg-strong':''}">انضم إلى المقعد ${lgNum(index+1)} · الفريق ${lgNum(index%2+1)} · ${lgNum(l.fee)} ذهب</button>`).join('')}</section>`;
}
function lgDetail(body){const l=LG.league,registered=l.mine==='paid',canJoin=l.status==='registration'&&!registered;
 ...
 if(LG.tab==='bracket'){
  const rounds=l.teams.length?Array.from({length:Math.ceil(Math.log2(l.teams.length))},(_,i)=>i+1):[];body.innerHTML=rounds.length?`${lgDrawInfo(l)}${lgBracketSummary(l)}<div class="lg-bracket-tools"><button data-lg="bracketScope" data-scope="current" aria-pressed="${LG.bracketScope!=='all'}">الدور الحالي</button><button data-lg="bracketScope" data-scope="all" aria-pressed="${LG.bracketScope==='all'}">المخطط كاملًا</button></div><p class="lg-note">اسحب المخطط لمتابعة التأهل. الجلسة التالية تبدأ بعد انتهاء مباريات الدور وفاصل 30 ثانية.</p><div class="lg-bracket">${rounds.filter(r=>LG.bracketScope==='all'||r===(Number(l.round)||1)).map(r=>`<section class="lg-round"><h3>${r===rounds.length?'النهائي':r===rounds.length-1?'نصف النهائي':'الدور '+lgNum(r)}</h3>${l.matches.filter(m=>m.round===r).map(m=>`<article class="lg-match"><div class="lg-team ${m.winner===m.teams[0]?'winner':''}">${lgTeam(l,m.teams[0],m)}</div><div class="lg-score">${m.bye?'تأهل مباشر':(m.snapshot?.points||[0,0]).map(lgNum).join(' : ')}</div>${m.bye?'':`<div class="lg-team ${m.winner===m.teams[1]?'winner':''}">${lgTeam(l,m.teams[1],m)}</div>`}<p class="lg-note">${m.done?'انتهت':Date.now()+LG.clockOffset<m.readyAt?'تبدأ بعد <b data-lg-time="'+m.readyAt+'">'+lgTime(m.readyAt)+'</b>':'المباراة جارية أو تنتظر اكتمال الحضور'}</p>${!m.done&&!m.bye?`<button data-lg="watch" data-match="${m.id}" style="width:100%">${lgSvg('eye')} شاهد المباراة</button>`:''}</article>`).join('')||'<article class="lg-match"><p>بانتظار الفرق المتأهلة</p><p class="lg-note">لم يبدأ هذا الدور بعد</p></article>'}</section>`).join('')}</div>`:`${lgDrawInfo(l)}<div class="lg-empty">${lgSvg('bracket')}<h2>القرعة عند إغلاق التسجيل</h2><p>الشركاء والفرق يُختارون تلقائيًا؛ كل فريق يضم لاعبين.</p></div>`;return;
 }
 const mine=l.assignment,watching=typeof ONL!=='undefined'&&ONL.active?l.matches.find(m=>m.id===ONL.room):null;
 body.innerHTML=lgSeatOptions(l,watching)+`<section class="lg-hero lg-detail-hero">${lgIcon()}<span class="lg-badge">${lgStatus[l.status]}</span><h2>${lgEsc(l.name)}</h2><p>${lgEsc(lgDate(l.start))} · السعودية</p>${l.status==='registration'?`<b data-lg-time="${l.start}">${lgTime(l.start)}</b>`:''}</section>${l.openAt&&l.status==='scheduled'?'<p class="lg-alert">يفتح التسجيل '+lgEsc(lgDate(l.openAt))+'</p>':''}${l.pending?'<p class="lg-alert">جارٍ استكمال المزامنة. المدفوعات محفوظة ولا تحتاج دفعًا جديدًا.</p>':''}<div class="lg-card">${lgRankRule(l,true)}<div class="lg-prizes"><div>🥇 لكل بطل<b>${lgNum(l.prizes.firstEach)}</b>ذهب + 1 دوري</div><div>🥈 لكل وصيف<b>${lgNum(l.prizes.secondEach)}</b>ذهب</div></div><div class="lg-data"><div><b>${lgNum(l.count)}/${lgNum(l.capacity)}</b><small>لاعبون مسجّلون</small></div><div><b>${lgNum(l.fee)}</b><small>الدخول · ذهب</small></div><div><b>${l.goal?lgNum(l.goal):'رزمة'}</b><small>هدف المباراة · نقاط</small></div></div>${lgDrawInfo(l)}${l.botCount?lgBracketSummary(l):''}<details class="lg-rules"><summary>نظام المشاركة والجوائز</summary><p class="lg-note">شريكك تختاره القرعة. عند الموعد تتحدد الأدوار حسب عدد المسجلين، ويُضاف الكمبيوتر لإكمال فريق عند العدد الفردي. الحد الأدنى مباراة واحدة من 4 مقاعد؛ ويمكن لمشاهد مؤهل شغل مقعد الكمبيوتر أثناء المباراة. يبدأ الدوري بوجود مسجل واحد على الأقل. الجوائز: 60% للفريق البطل و30% للوصيف، بالتساوي بين المقعدين. حصة الكمبيوتر لا تُصرف، ولا يحصل على لقب. رسوم الإنشاء خارج الجوائز.</p></details>${l.code?`<div class="lg-share"><label for="lgShareCode">كود المشاركة</label><input id="lgShareCode" readonly dir="ltr" value="${lgEsc(l.code)}"><button data-lg="copyCode" aria-label="نسخ كود المشاركة">${lgSvg('copy')}</button></div>`:''}${mine?`<p class="lg-alert">فريقك جاهز. ${Date.now()+LG.clockOffset<mine.readyAt?'الفاصل المتبقي <b data-lg-time="'+mine.readyAt+'">'+lgTime(mine.readyAt)+'</b>':'ادخل طاولتك الآن · مهلة الحضور 90 ثانية'}</p><button data-lg="enter" class="lg-strong" style="width:100%">العودة إلى مباراة فريقك</button>`:''}${['finished','paying'].includes(l.status)?'<h3>أبطال الدوري</h3>'+lgTeam(l,l.champion):''}${l.owner===FB.user?.uid&&l.status==='registration'?'<p><button data-lg="cancelLeague">إلغاء الدوري · رسوم الإنشاء لا تُسترد</button></p>':''}</div><h3>المشاركون</h3><div class="lg-people">${l.entries.map(p=>'<div class="lg-card">'+lgPerson(p)+'</div>').join('')||'<p>لا يوجد مسجلون بعد.</p>'}</div>`;
...
function lgHelp(){document.getElementById('lgBody').innerHTML='<section class="lg-card"><h3>طريقك إلى اللقب</h3><p>يمكنك التسجيل في دوري واحد فقط. ألغِ مشاركتك قبل بدايته أو انتظر انتهاءه لتسجل في دوري آخر.</p><p>تسجل فرديًا، وتُختار الفرق بالقرعة. كل فريق لاعبان، والخاسر يخرج. عند الموعد تُبنى الأدوار على عدد المسجلين الفعلي، بحد أدنى فريقين. يُضاف الكمبيوتر لإكمال العدد الفردي أو الحد الأدنى لمباراة واحدة، ويستطيع المشاهدون شغل مقعده لاحقًا. إذا كان عدد الفرق لا يملأ المخطط، تمنح القرعة تأهلًا مباشرًا لبعض الفرق.</p><p>بين الأدوار 30 ثانية. مهلة دخول الطاولة 90 ثانية؛ إذا انتهت يكمل الكمبيوتر مكانك ولا تعود للجولة نفسها. أثناء اللعب، الغياب يتحول إلى تحكم تلقائي، وبعد 3 أدوار لعب يستبدلك الكمبيوتر. يستطيع مشاهد مؤهل دخول مقعد الكمبيوتر برسوم الدخول.</p><p>الشريك الموجود يكمل مع الكمبيوتر. إذا غاب الفريق كاملًا، مهلة 90 ثانية قبل الخسارة بالانسحاب.</p><p>تبدأ صدارة جديدة الساعة 00:00 السعودية أول كل شهر. إجمالي ألقابك يبقى دائمًا.</p></section>';}
function lgClick(ev){
 ...
 else if(a==='bracketScope'){LG.bracketScope=b.dataset.scope==='all'?'all':'current';lgRender();}
 ...
 else if(!LG.busy&&a==='participate'){
  if(LG.league.assignment){lgActionError('لديك مقعد في هذا الدوري بالفعل. ادخل مباراة فريقك من نظرة عامة.');return;}
  const intent={league:LG.league.id,match:b.dataset.match,index:Number(b.dataset.index),uid:FB.user?.uid,expires:Date.now()+120000};LG.seatIntent=intent;
  lgAct('watch',{match:intent.match}).then(x=>{if(!x&&LG.seatIntent===intent)LG.seatIntent=null;});
 }
...
function lgRenderLobby(msg){
 const t=msg.league;if(!t)return;
 const root=document.getElementById('onl'),wait=document.getElementById('onlWait'),lb=msg.lobby||{},seats=msg.seats||[];
 root.dataset.league='true';root._lgLobbyChanged=true;
 const spectator=!!msg.spectateOnly||!seats.includes(msg.you);
 wait.querySelector('h2').textContent=spectator?'مشاهدة مباراة الدوري':'انتظار مباراة الدوري';
 wait.querySelector('.sub').textContent=spectator?'أنت مشاهد · لم تنضم كلاعب ولم تُخصم رسوم مشاركة':'مقعدك وشريكك محددان بالقرعة';
 const names=lb.names||{};
 document.getElementById('lobbyPlayers').innerHTML=[0,1].map(team=>`<section class="lg-wait-team"><strong>الفريق ${team+1}</strong>${[team,team+2].map(index=>{
  const pid=seats[index],bot=!!pid&&pid.startsWith('bot-');
  // lobby names have already been escaped once by oHandle.
  return `<div class="lg-wait-player"><span>${bot?'كمبيوتر':names[pid]||'بانتظار اللاعب'}</span><small>${bot?'يمكن طلب المشاركة بعد بدء المباراة':'مقعد محجوز بالقرعة'}</small></div>`;
 }).join('')}</section>`).join('');
 const hint=Date.now()+LG.clockOffset<t.readyAt?'المباراة لم تبدأ · '+lgTime(t.readyAt):'بانتظار حضور اللاعبين · مهلة الحضور 90 ثانية';
 document.getElementById('lobbyHint').textContent=hint+(spectator?' · ستبدأ المشاهدة تلقائيًا':'');
 let back=document.getElementById('lgLobbyBack');if(!back){back=document.createElement('button');back.id='lgLobbyBack';back.textContent='العودة إلى مخطط الدوري';wait.appendChild(back);}back.onclick=()=>lgOpen(t.id,'bracket');
}
function uiLeagueState(msg){
 lgUpdateSeatButton(msg);
 const intent=LG.seatIntent;
 if(intent&&(intent.uid!==FB.user?.uid||Date.now()>intent.expires))LG.seatIntent=null;
 else if(intent&&ONL.active&&ONL.room===intent.match&&msg.league?.id===intent.league&&!LG.open){
  if(lgCanRequestSeat(msg)){LG.seatChoice=intent;LG.seatIntent=null;lgOpen(intent.league);}
  else if(msg.status!=='lobby'){LG.seatIntent=null;toast2('لا يوجد مقعد متاح لك في هذه المباراة حاليًا. يمكنك متابعة المشاهدة.');}
 }
 const t=msg.league,e=document.getElementById('onl');e.dataset.league=t?'true':'false';if(!t){document.getElementById('lgLobbyBack')?.remove();document.getElementById('uiOnlineTitle').onclick=null;document.getElementById('lgResultBack')?.remove();if(e._lgLobbyChanged){const wait=document.getElementById('onlWait');wait.querySelector('h2').textContent='طاولة الأصدقاء';wait.querySelector('.sub').textContent='أرسل الرابط لأصدقائك (حتى 3) — والبوتات تكمّل الناقص';e._lgLobbyChanged=false;}return;}
 ...
 if(msg.status==='lobby')lgRenderLobby(ONL);
